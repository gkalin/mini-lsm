...
impl Key<Vec<u8>> {
    ...
    pub fn decode(slice: Vec<u8>, first: KeySlice) -> Self {
        let ts_slice: [u8; 8] = slice[slice.len() - 8..].try_into().unwrap();
        let ts = u64::from_le_bytes(ts_slice);
        let rest_key = &slice[2..slice.len() - 8];
        let prefix_len = u16::from_le_bytes(slice[0..2].try_into().unwrap()) as usize;
        let mut key = Vec::new();
        let overlap = first.key_ref();
        key.extend_from_slice(&overlap[0..prefix_len]);
...
impl<'a> Key<&'a [u8]> {
    ...
    pub fn prefix_encode(self, first: KeySlice) -> KeyVec {
        let prefix_len = self
            .0
            .iter()
            .zip(first.key_ref().iter())
            .take_while(|(x, y)| x == y)
            .count();
        let mut encoded = Vec::new();
        encoded.extend_from_slice(&(prefix_len as u16).to_le_bytes());
        encoded.extend_from_slice(&self.0[prefix_len..]);
        Key(encoded, self.1)
    }
}
